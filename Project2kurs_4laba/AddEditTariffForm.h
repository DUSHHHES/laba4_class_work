...
namespace Project2kurs_4laba { 
	...
	public:
		...
		AddEditTariffForm(void)// Создаёт пустую форму
		{
			InitializeComponent();
			this->Text = "Добавить новый тариф";
			// Заполняем ComboBox значениями из перечисления CargoType
			comboBoxCargoType->DataSource = Enum::GetValues(CargoType::typeid);
		}
		...
		AddEditTariffForm(Tariff^ tariffToEdit)// Заполняет форму дял редактирования + меняет заголовок формы
		{
			...
			comboBoxCargoType->SelectedItem = ResultTariff->Type;
		}
...
#pragma region Windows Form Designer generated code
		void InitializeComponent(void)
		{
			...
			this->ClientSize = System::Drawing::Size(524, 321);
...
#pragma endregion
	private: System::Void buttonOK_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		// Проверка ввода цены
		String^ priceText = textBoxPrice->Text->Replace(",", "."); // Заменяем запятую на точку
